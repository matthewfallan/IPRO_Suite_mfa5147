...
def OptCDR_info(experiment, data):
    ...
    for user, computer in zip(['Home Directory', 'Canonical Folder',  
                               ...
                               'Framework Reference'], ['Optcdr Path',  
    ...
    for name in ['Optcdr Canonical Path', 'Optcdr Clash Path',  
                 'Optcdr Gaussian Path', 'Optcdr Cluster Folder',
                 'Optcdr Usage Pattern', 'Optcdr Framework Reference']:
    ...
    # Load the OptCDR rotation information
    try:
        rotations = data['Antigen Rotation'].lower()
    except IPROError as error:
            errors += str(error)
    experiment["Optcdr Rotations"] = rotations
    # Load the antibody chains to be designed
    errors += standard_load('Optcdr Chains', experiment, data, 'list')
    # Load the antigen initial positions and library size
    for name in ['Optcdr Positions', 'Optcdr Libraries']:
        errors += standard_load(name, experiment, data, 'integer')
    return errors
...
def EpitopePositions(experiment, data):
    ...
    # Store errors here
    errors = ''
    # If the Epitope Position information is missing, store an empty dictionary
    if "Epitope Position" not in data:
        experiment["Epitope Positions"] = {}
    else:
        # if the information is a single string, put it in a list
