...
def copy_cpp_files(rotamer = True, docking = True, optcdr = False, 
                   folder = "./"):
    ...
    # If the OptCDR scoring function is needed
    if optcdr:
        files.append("scoring.out")
...
def copy_standard_files(experiment, folder = None, cpp = True, ff = True, \
                        solv = False, optcdr = False):
    ...
    if cpp:
        copy_cpp_files(True, True, optcdr, experiment["Folder"])
    if ff:
...
def update_Current(experiment, folder, gn = None):
    ...
    for group in experiment:
        ...
        for molecule in group:
            ...
            # Load those contents
            #FIXME
            print("update_Current", os.getcwd())
            f = open(name, "r")
            lines = f.readlines()
            f.close()
...
def output_best(experiment, iteration, gn = None):
    ...
    if experiment["Type"] == "OptCDR":
        output_scores(experiment, folder)
    else:       
        output_Energies(experiment, folder, gn)

def format_scores(experiment, index = 0, simple = True):
    """Create the text listing the scores and corresponding movements for a
    library."""
    # A list of the expected CDRs
    cdrs = []
    # Create the list using the information about the OptCDR Chains to be used
    ...
    # Create the formatted text
    text = ''
    # Include the score for the movement
    if simple:
        label = "Score:"
        text += label.ljust(10) + str(experiment["Scores"][index][1]["Score"]) + "\n"
    else:
        text += "The score for the set of canonical structures used is "
        text += str(experiment["Scores"][index][1]["Score"]) + "\n"
    # Include the canonical structure information
    for num, cdr in enumerate(cdrs):
        if simple:
            label = cdr + ":"
            text += label.ljust(10) + str(experiment["Scores"][index][1][num+1]) + "\n"
        else:
            text += "The " + cdr + " CDR used canonical structure #" 
            text += str(experiment["Scores"][index][1][num+1]) + "\n"
    # Include the movement information
    entries = experiment["Movements"][experiment["Scores"][index][0]]
    # Determine the coordinates used within the experiment
    coors = ["X", "Y", "Z"]
    # Account for both translations and rotations in each direction
    end = len(entries)/2
    coors = coors[:end + 1]
    count = -1
    # Go through each of the entries
    # Translations appear first, then rotations
    for movement in ["TRANSLATION", "ROTATION"]:
        for coor in coors:
            count += 1
            if simple:
                label = coor + " " + movement + ":"
                text += label.ljust(20) + format(entries[count], '.4f') + "\n"
            else:
                text += "The antigen was " + movement[:-3] + "ED by "
                text += format(entries[count], '.3f') 
                if movement == "TRANSLATION":
                    text += " Angstroms "
                else:
                    text += " Radians "
                text += "in the " + coor + " direction\n"      
    return text

def output_scores(experiment, folder, ln = None):
    """Output the relevant contents of the Scores List and Movements dictionaries"""
    # If a library was specified, output the scores for that library
    if ln != None:
        # Create a file name for the information
        name = folder + "Library" + str(ln) + "_Canonicals.txt"
        # Write the text
        text = format_scores(experiment, ln-1)
        f = open(name, "w")
        f.write(text)
        f.close()
    # Otherwise, loop through the libraries
    else:
        for i in range(1, experiment["Optcdr Libraries"] + 1):
            # Create a file name for the information
            name = folder + "Library" + str(i) + "_Canonicals.txt"
            # Write the text
            text = format_scores(experiment, i-1)
            f = open(name, "w")
            f.write(text)
            f.close()

def load_scores(experiment, folder):
    """Load the random initial position information and the corresponding
    canonical structures"""
    # Store the antigen positioning information in this dictionary
    movements = {}
    # Store the canonical structure information in this list
    scores = []
    # Load the information for the libraries
    for i in range(1, experiment["Optcdr Libraries"] + 1):  
        # Open the appropriate file 
        f = open(folder + "Library" + str(i) + "_Canonicals.txt")
        # Gather the lines in the file
        lines = f.readlines()
        # Close the file
        f.close()
        # Get a list of the CDRs to be included
        cdrs = []
        # Create the list using the information about the OptCDR Chains to be used
        for chain in experiment['Optcdr Chains']:
            for j in range(1, 4):
                cdr = chain[0].upper() + str(j)
                cdrs.append(cdr)
        cdrs.sort()
        # Go through the data and update the data structures accordingly
        canonical_data = {}
        moves = []
        for line in lines:
            items = line.split()
            # If there are 2 entries in the line, it should be used for the
            # canonical structure information
            if len(items) == 2:
                key = items[0][:-1]
                value = items[1]
                # Validate that the key is acceptable
                if key in ["H1", "H2", "H3", "L1", "L2", "L3", "Score"]:
                    # Replace CDR names with their numerical index
                    if key != "Score":
                        key = cdrs.index(key) + 1
                    canonical_data[key] = int(value)
            # Otherwise, it should have three entries 
            else:
                moves.append(float(items[-1]))
        # Add this library-specific information to the complete compilation of
        # data
        scores.append([i, canonical_data])
        movements[i] = moves
    # Put all the data structures into the Experiment class object
    experiment["Movements"] = movements
