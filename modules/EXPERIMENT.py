...
def input_validation(experiment, isOptcdr = False):
...
moment, so please be patient."""
    ...
    # If this is a standard input validation, make the Experiment's folder
    if not isOptcdr:
    ...
    # If this is a framework validation, keep track of the current files
    if isOptcdr:
    ...
    # energy parameters
    molecules = appropriateness(experiment, True, isOptcdr)
    # If everything worked correctly, the folder can be prepped for the
    # experiment
    names = os.listdir("./")
    # If this is a standard input validation
    ...
    if not isOptcdr:
        ...
        # Copy in the force field and solvation files
        SHARING.copy_standard_files(experiment, current + "/input_files/", \
                                False, True, True)
        ...
        os.chdir(current)
    # If this is a framework validation
    else:
        # only delete the newly constructed files
        for name in names:
            if name not in currentList:
                i = os.remove(name)   
        # Go into the structures folder
        os.chdir("structures")
        # Output each Molecule in the structures folder
        for molecule in molecules:
            molecule.output(None, experiment["File Format"], experiment["User"])
...
class Experiment(object):
    ...
    def __setattr__(self, name, value):
        ...
        "Docking Groups", "Activity", "Optzyme Information", "Movements", \
        "Scores"]:
        ...
        'Optcdr Framework Reference', 'Optcdr Positions', 'Optcdr Libraries', \
        'Optcdr Usage Pattern']:
    ...
    def load_from_file(self):
        ...
        if self["Type"] in ["OptCDR"]:
            ...
            errors += IO_LOADING.EpitopePositions(self, data)
            errors += IO_LOADING.OptCDR_frameworks(self, data)
    ...
    def user_creation(self):
        ...
        if self["Type"] in ["OptCDR"]:
            ...
            IO_ASK.Epitope(self)
        if self["Type"] in ["OptGraft"]:
            IO_ASK.scaffold_molecule(self)
            IO_ASK.ligand_molecules(self)
        #if self["Type"] in ["OptMAVEn"]:
        #    IO_ASK.antigen_molecules(self)
        if self["Type"] in ["IPRO", "Mutator", "OptCDR", "OptGraft", \
        "OptMAVEn", "OptZyme"]:
            if self["Force Field"] == "CHARMM":
        ...
        if self["Type"] in ["OptCDR"]:
            ...
            input_validation(self, True)
        if self["Type"] in ["IPRO", "OptCDR", "OptGraft", "OptMAVEn", \
        "OptZyme"]:
    ...
    def finish_creation(self, load = True):
        ...
        # If this is an OptCDR experiment, create the Movements dictionary and 
        # Scores list
        if self["Type"] in ["OptCDR"]:
            self["Movements"] = {}
            self["Scores"] = []
