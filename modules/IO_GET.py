    ...
    while not accept:
        ...
        if do:
            ...
            if len(residues) == 1:
                ...
                question += " Residues between and inclusive of " + residues[0] 
                question += " and " + residues[1] + " should be excluded from "
...
def PermittedKinds(permitted, experiment):
    ...
    moleculeKeys = list(experiment["Design Positions"].keys())
    moleculeKeys.sort()
        ...
        while not accept:
            ...
            if pick:
                ...
                # Otherwise, ask
                    ...
                    words = list(propertiesAA[ff].keys())
                    words.sort()
                    for word in words:
                        available = []
        ...
        if len(residue) == 1:
            ...
            for atom in residue:
                if atom.name not in exclude:
                    options.append(atom.name)
            print(screen_formatting(MOLECULES.summarize(residue, exclude)))
            question = "Which Atom contains Atom " + str(len(atoms) + 1)
            question += " for the dihedral restraint?"
            # Get the answer
