...
def OptCDR_info(experiment):
    ...
    for LHS, name in zip(['Canonical Folder', 'Clash File', 'Position File',
                           'Cluster Folder', 'Usage Pattern',
                           'Framework Reference'], ['Optcdr Canonical Path',
                           'Optcdr Clash Path', 'Optcdr Gaussian Path',
                           'Optcdr Cluster Folder', 'Optcdr Usage Pattern',
                           'Optcdr Framework Reference']):
        value = experiment[name].replace(experiment['Optcdr Path'], '') 
        text += standard_format(LHS, value)
    # Go through the other attributes
    for name in ['Optcdr Chains', 'Optcdr Positions', 'Optcdr Libraries']:
        text += standard_format(name, experiment[name])
    # Format the name of the 'Optcdr Rotations' attribute differently
    LHS = "Antigen Rotation"
    RHS = experiment['Optcdr Rotations'].capitalize()     
    ...
    for chain in experiment['Optcdr Frameworks']:
        ...
        if experiment['Optcdr Frameworks'][chain][0][1] == ' ':
            RHS += "_"
        else:
            RHS += experiment['Optcdr Frameworks'][chain][0][1]
        RHS += " from file " + experiment['Optcdr Frameworks'][chain][0][0]
        RHS += " is Design Molecule "  
        RHS += experiment['Optcdr Frameworks'][chain][0][2].name
...
def EpitopePositions(experiment):
    ...
    # Store the text here
    text = ''
    if "Type" in experiment and experiment["Type"] in \
    ['OptMAVEn', 'OptCDR'] and experiment["Epitope Positions"] != {}:
